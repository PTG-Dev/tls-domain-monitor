...
class Menu:
    ...
    def _print_help(self) -> None:
        ...
        print(
            ...
            f"{sep}\n"
            f"     {_D}-k{_R}  {_DI}--keyword{_R}   {_DI}Filter — only show matching domains{_R}\n"
            f"     {_DI}ex:{_R}  scan -k bank\n"
            f"\n"
            f"     {_D}-l{_R}  {_DI}--limit{_R}     {_DI}Stop after N unique domains{_R}\n"
            f"     {_DI}ex:{_R}  scan -l 20\n"
            f"\n"
            f"     {_D}-r{_R}  {_DI}--http{_R}      {_DI}Show HTTP response code per domain{_R}\n"
            f"     {_DI}ex:{_R}  scan -r\n"
            f"\n"
            f"     {_D}-e{_R}  {_DI}--exclude{_R}   {_DI}Skip domains containing this word{_R}\n"
            f"     {_DI}ex:{_R}  scan -e cdn\n"
            f"\n"
            f"         {_DI}--cert{_R}      {_DI}Show cert info — issuer + expiry{_R}\n"
            f"     {_DI}ex:{_R}  scan --cert\n"
            f"\n"
            f"     {_D}-v{_R}  {_DI}--verbose{_R}   {_DI}With --cert: also show org, country, SAN count{_R}\n"
            f"     {_DI}ex:{_R}  scan --cert -v\n"
            f"\n"
            f"     {_D}-i{_R}  {_DI}--ip{_R}        {_DI}Resolve and show the IP address of each domain{_R}\n"
            f"     {_DI}ex:{_R}  scan -i\n"
            f"\n"
            f"     {_D}-o{_R}  {_DI}--output{_R}    {_DI}Save results to a timestamped file in outputlogs/{_R}\n"
            f"     {_DI}ex:{_R}  scan -o\n"
            f"\n"
            f"     {_DI}combine:{_R}  scan -k bird -e cdn -l 10 -r --cert -v -i -o\n"
            f"{sep}\n"
        )
